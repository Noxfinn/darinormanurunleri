import dotenv from "dotenv";
dotenv.config({ path: ".env.local" });

import fs from "fs";
import path from "path";
import { createClient } from "@sanity/client";

if (!process.env.SANITY_WRITE_TOKEN) {
  throw new Error("SANITY_WRITE_TOKEN bulunamadı. .env.local kontrol et.");
}

const client = createClient({
  projectId: "y50junbp",
  dataset: "production",
  apiVersion: "2025-05-15",
  token: process.env.SANITY_WRITE_TOKEN,
  useCdn: false
});

const products = JSON.parse(
  fs.readFileSync("data/edge-bands.json", "utf8")
);

async function getCategoryRef() {
  const category = await client.fetch(
    `*[_type == "category" && slug.current == "kenar-bandi"][0]{_id}`
  );

  if (!category?._id) {
    throw new Error("Kenar Bandı kategorisi bulunamadı. Slug kenar-bandi olmalı.");
  }

  return category._id;
}

async function uploadImage(localImagePath?: string) {
  if (!localImagePath) return null;

  const cleanPath = localImagePath.startsWith("/")
    ? localImagePath.slice(1)
    : localImagePath;

  const fullPath = path.join(process.cwd(), "public", cleanPath.replace("imported/", "imported/"));

  if (!fs.existsSync(fullPath)) {
    console.log("Görsel bulunamadı:", fullPath);
    return null;
  }

  const imageAsset = await client.assets.upload(
    "image",
    fs.createReadStream(fullPath),
    {
      filename: path.basename(fullPath)
    }
  );

  return imageAsset._id;
}

async function main() {
  console.log("Import başlıyor...");
  console.log("Ürün sayısı:", products.length);

  const categoryId = await getCategoryRef();

  for (const item of products) {
    const existing = await client.fetch(
      `*[_type == "product" && slug.current == $slug][0]{_id}`,
      { slug: item.slug }
    );

    if (existing?._id) {
      console.log("Zaten var:", item.titleTR);
      continue;
    }

    const assetId = await uploadImage(item.image);

    await client.create({
      _type: "product",
      titleTR: item.titleTR,
      titleEN: item.titleEN,

      slug: {
        _type: "slug",
        current: item.slug
      },

      category: {
        _type: "reference",
        _ref: categoryId
      },

      subCategory: item.subCategory,
      collection: item.collection,
      model: item.model,

      descriptionTR: `${item.titleTR} ${item.subCategory} kenar bandı ürünüdür.`,
      descriptionEN: `${item.titleEN} ${item.subCategory} edge band product.`,

      mainImage: assetId
        ? {
            _type: "image",
            asset: {
              _type: "reference",
              _ref: assetId
            }
          }
        : undefined
    });

    console.log("Eklendi:", item.titleTR);
  }

  console.log("Import tamamlandı.");
}

main();